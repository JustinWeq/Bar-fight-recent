    ...
    public class Room
    {
        ...
        public void loadCombatants(ContentManager content,string[] selectedCombatants, int numOfPlayers)
        {
            ...
            for (int i = 0; i < numOfPlayers; i++)
            {
                ...
                player.X = barX + (barWidth / 4 * i);
                player.Z = barY;
                player.LastX = barX + (barWidth / 4 * i);
                player.LastZ = barY;
        ...
        public void update(GameTime time)
        {
            ...
            else
            {
                if (!hasWon)
                {
                    if(parent.Music)
                    winSong.Play();
                    view = Matrix.CreateLookAt(new Vector3(combatants[winner].X +(float) (Math.Cos(combatants[winner].RotY-MathHelper.ToRadians(90))*20), combatants[winner].Y + 10, combatants[winner].Z +(float) (Math.Sin(-combatants[winner].RotY - MathHelper.ToRadians(90)) *20)),
                        new Vector3( combatants[winner].X,combatants[winner].Y,combatants[winner].Z),Vector3.UnitY);
                }
                hasWon = true;
                instance.Stop();
                int i = 0;
                    ...
                    case "DrinkSpawner":
                        {
                            ...
                            drinkSpawner.ORIGEN.Type = Origen.Collision_Type.AABB;
                            drinkSpawner.X = X;
                            drinkSpawner.Y = Y;
                            drinkSpawner.Z = Z;
