...
namespace BarFightEngineM2
{
    class CharacterSelection
    {
        ...
        int timer;
        public CharacterSelection(BarFight parent,GraphicsDevice device)
        {
                    ...
                    case 0:
                        {
                            if (state.Buttons.X == ButtonState.Pressed)
                            {
                                if (playerIndex == numOfPlayers-1)
                                {
                                    ...
                                    playerIndex = 0;
                                    parent.gameState = BarFight.GameState.InGame;
                                    parent.stopSound();
                    ...
                    case 1:
                        {
                            if (state.Buttons.X == ButtonState.Pressed)
                            {
                                ...
                                if (playerIndex == 0)
                                {
                                    parent.gameState = BarFight.GameState.LevelSelection;
                                    parent.resetTimers();
                                }
                                else
                                {
