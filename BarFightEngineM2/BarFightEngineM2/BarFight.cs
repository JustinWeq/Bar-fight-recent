...
namespace BarFightEngineM2
{
    ...
    /// </summary>
    public class BarFight : Microsoft.Xna.Framework.Game
    {
        ...
        private static Settings defualts = new Settings();
        private bool sfx;
        public bool Sfx
        {
            get { return sfx; }
        ...
        public void applySettings(Settings setting)
        {
            graphics.IsFullScreen = setting.fullscreen;
            
            graphics.PreferMultiSampling = setting.AA;
            if (setting.width > defualts.width)
               graphics.PreferredBackBufferWidth = screenWidth  = defualts.width;
            else
                graphics.PreferredBackBufferWidth = screenWidth = setting.width;

            if (setting.height > defualts.height)
                graphics.PreferredBackBufferHeight = screenHeight = defualts.height;
            else
                graphics.PreferredBackBufferHeight = screenHeight = setting.height;
        ...
        protected override void LoadContent()
        {
            ...
            //set up the defualts settings
            defualts = new Settings();
            defualts.AA = true;
            defualts.height = GraphicsDevice.Viewport.Height;
            defualts.width = GraphicsDevice.Viewport.Width;
            defualts.sfx = true;
            defualts.music = true;
            defualts.rumble = true;

            applySettings(defualts);
                ...
                case GameState.Options:
                    {
                        ...
                        options.update(padState);
                        break; 
                    }
                case GameState.Controls:
                    {
                        GamePadState padState = GamePad.GetState(PlayerIndex.One);
                        controls.update(padState);
